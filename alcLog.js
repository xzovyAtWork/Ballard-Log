            ...
            class Device{
                ...
                checkPrevious(){
                    ...
                    this.retrievedValues.forEach((e) => {
                        if(between(this.getValue(), e, 1.5)){
                            validity = true;
                        }
                    })
                    return validity;
                ...
                updateRetrievedValues(delay = 3000){
                    let item = this;
                    return new Promise((resolve, reject)=>{
                        console.log(between(item.getValue(item.fbkValueID), item.getValue(item.cmdValueID), 2.5), !item.checkPrevious())
                        if((between(item.getValue(item.fbkValueID), item.getValue(item.cmdValueID), 2.5)) && !item.checkPrevious()){
                            setTimeout(()=>{
                                item.retrievedValues.push(parseFloat(item.getValue(item.fbkValueID)))
                                lastPushed = item;
                                console.log(item.name, item.retrievedValues)
                                if(item.name == 'VFD'){
                                    console.log("airflow:",airflow.getValue())
                                };
                                resolve(`${item.loggedStatus}`);
                            }, delay);
                        }
            ...
            function strokeAnalogDevice(device, withOutput = false, commandValue){
                return new Promise( function(resolve, reject){
                    ...
                    let watchdog = setInterval( ()=>{
                        ...
                        if(between(parseFloat(device.getValue()), commandValue)){
                            clearInterval(watchdog);
                            console.log('stopped')
                            resolve('in position');
                        }
        ...
        async function testDamper(device, commandValues){
            return new Promise((resolve, reject) =>{
                let tested = resolve;
                device.updateRetrievedValues();
                strokeAnalogDevice(device, true, commandValues[0]).then(()=>{
                    device.updateRetrievedValues().then(()=>{
