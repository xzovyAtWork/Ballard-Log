    ...
    */
    ...
    class Device{
        ...
        updateLoggedStatus(type = 'binary'){
            if(type == 'binary' && (this.getValue(this.fbkValueID) !== this.loggedStatus)){
                this.loggedStatus = this.getValue(this.fbkValueID)
                console.log(`${this.name} is ${this.loggedStatus}`)
            }else if(type == 'analog' && ((between(this.getValue(this.fbkValueID), this.getValue(this.cmdValueID), 2.5)) && !this.checkPrevious())){
                setTimeout(()=>{
                    this.retrievedValues.push(parseFloat(this.getValue(this.fbkValueID)))
                    console.log(this.name,this.retrievedValues);
                    lastPushed = this;
                    if(this.name == 'VFD'){
                        console.log("airflow:",airflow.feedback)
                    }
                }, 4000);
            }
    ...
    //analog IO's
    let faceDamper = new Device('Face Damper', [414, 1964, 1975] );
    let bypassDamper = new Device ('Bypass Damper', [458, 2008, 2019]);
    let vfd = new Device('VFD', [766, 1698, 1709]); //speed command

    //analog I's
    let conductivity = new Device('Conductivity', 502);
    let maTemp = new Device('M/A', 546);
    let saTemp = new Device('S/A', 103);
    let rh1 = new Device('RH One', 590);
    let rh2 = new Device('RH Two', 634);
    ...
    let leak2 = new Device('Mech. Gallery Leak Detector', 1118);

    let primary = new Device('UPS Primary Status', 1582)
    let secondary = new Device('Secondary Status', 1611)

    let vfdHOA = new Device('VFD HOA', [1524, 2178]); // vfd enable
    let vfdFault = new Device('VFD Fault', 1553);
    ...
    (function fetchStatusOnLoad(){
        sensorList.forEach(e => {e.getValue()})
        binaryDeviceList.slice(2).forEach(e => e.getValue())
        faceDamper.getValue();
        bypassDamper.getValue();
